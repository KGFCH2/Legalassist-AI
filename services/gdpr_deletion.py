...
class GDPRDeletionService:
    ...
    def export_user_data_before_deletion(self, user_id: int, db: Session) -> Tuple[Optional[Dict[str, Any]], str]:
        ...
        self._log_audit(
            ...
            outcome="success",
            details={"case_ids": case_ids, "deletion_token": deletion_token},
        )
        return export_data, deletion_token

    def delete_user_data(self, user_id: int) -> DeletionResult:
        result = DeletionResult(user_id=user_id, success=False, steps=[])
        db = self._get_db()
        try:
            ...
            try:
                ...
                self._complete_step(export_step, {"cases_exported": len(export_data.get("cases", [])), "deletion_token": deletion_token})
            except Exception as e:
            ...
            try:
                ...
                self._complete_step(case_step, {"cases_deleted": deleted_cases, "deadlines_deleted": deleted_deadlines})
            except Exception as e:
    ...
    def _redact_user_data(self, db: Session, user_id: int) -> int:
        ...
        if user:
            ...
            db.commit()
            redacted_count += 1
            self._log_audit("gdpr.pii_redacted", "redact_user_pii", user_id, "user", str(user_id), "success", {"fields": ["email", "full_name", "phone", "address"]})
    ...
    def _delete_user_vectors(self, user_id: int, db: Session) -> int:
        ...
        try:
            ...
            db.query(CaseEmbedding).filter(CaseEmbedding.case_id.in_(case_ids)).delete(synchronize_session=False)
        except Exception as e:
            logger.warning("Vector store deletion failed: %s", e)
        db.commit()
        self._log_audit("gdpr.vectors_deleted", "delete_vector_embeddings", user_id, "user", str(user_id), "success", {"vectors_deleted": deleted_count, "case_ids": case_ids})
    ...
    def _delete_user_attachments(self, user_id: int, db: Session) -> int:
        ...
        for attachment in attachments:
            ...
            deleted_count += 1
        db.query(Attachment).filter(Attachment.case_id.in_(case_ids)).delete(synchronize_session=False)
        db.commit()
        self._log_audit("gdpr.attachments_deleted", "delete_attachments", user_id, "user", str(user_id), "success", {"attachments_deleted": deleted_count})
        return deleted_count
    ...
    def _delete_user_timeline_and_notes(self, user_id: int, db: Session) -> int:
        ...
        cases = db.query(Case).filter(Case.user_id == user_id).all()
        case_ids = [c.id for c in cases]
        deleted = db.query(CaseTimeline).filter(CaseTimeline.case_id.in_(case_ids)).delete(synchronize_session=False)
        deleted_count += deleted
        notes = db.query(CaseNote).filter(CaseNote.user_id == user_id).all()
        note_ids = [n.id for n in notes]
        if note_ids:
            db.query(CaseNoteVersion).filter(CaseNoteVersion.case_note_id.in_(note_ids)).delete(synchronize_session=False)
        deleted = db.query(CaseNote).filter(CaseNote.user_id == user_id).delete(synchronize_session=False)
        deleted_count += deleted
        db.commit()
        self._log_audit("gdpr.timeline_deleted", "delete_timeline_and_notes", user_id, "user", str(user_id), "success", {"events_deleted": deleted_count})
        return deleted_count

    def _delete_user_cases_and_deadlines(self, user_id: int, db: Session) -> Tuple[int, int]:
        cases = db.query(Case).filter(Case.user_id == user_id).all()
        case_ids = [c.id for c in cases]
        deleted_deadlines = db.query(CaseDeadline).filter(CaseDeadline.user_id == user_id).delete(synchronize_session=False)
        db.query(CaseDocument).filter(CaseDocument.case_id.in_(case_ids)).delete(synchronize_session=False)
        deleted_cases = db.query(Case).filter(Case.user_id == user_id).delete(synchronize_session=False)
        db.commit()
        self._log_audit("gdpr.cases_deleted", "delete_cases_and_deadlines", user_id, "user", str(user_id), "success", {"cases_deleted": deleted_cases, "deadlines_deleted": deleted_deadlines})
    ...
    def _finalize_user_deletion(self, user_id: int, db: Session) -> None:
        ...
        if user:
            ...
            db.commit()
        self._log_audit("gdpr.deletion_finalized", "finalize_deletion", user_id, "user", str(user_id), "success")
...
def verify_deletion_audit_trail(user_id: int) -> Dict[str, Any]:
    ...
    with SessionLocal() as db:
        gdpr_events = db.execute(
            f"SELECT * FROM immutable_audit_log WHERE actor_user_id = {user_id} AND (event_type LIKE 'gdpr.%' OR action LIKE 'gdpr%')"
        ).fetchall()
    return {
